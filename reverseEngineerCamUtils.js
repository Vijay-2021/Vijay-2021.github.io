   ...
   */
   ...
   var e="function"==typeof Object.defineProperties?Object.defineProperty:function(a,b,c){
    ...
    function f(a){
        ...
        }throw Error("Cannot find global object");//so if we don't return c, then throw an error meaning that there is no global obj?
    }
    var h=f(this);//so h uses the function f, on the global object
    function k(a,b){
        if(b)a:{
            var c=h;a=a.split(".");
            for(var d=0;d<a.length-1;d++){var g=a[d];if(!(g in c))break a;
                c=c[g]}
            a=a[a.length-1];
            d=c[a];b=b(d);
    ...
    function p(a,b){this.video=a;this.h=0;//set frame time checker to 0, assign a to video 
    ...
    function r(a){
        ...
        b?b.then(function(){q(a)}):q(a) //so  if b exists, then use b.then, else just do q(a). and b is the onframe method, so we run b, then we call the funciton again! okay!
    }
